...
| --- | --- | --- |
| `ignoreCharacters` | `true` | Bỏ qua rule đếm số ký tự của title và description. Khi `true`, plugin sẽ không chấm `TITLE_CHARACTERS` và `DESCRIPTION_CHARACTERS`, trừ khi bạn bật/tắt trực tiếp trong `criteria`. |
| `ignoreSeoHeading` | `false` | Bỏ qua toàn bộ nhóm heading SEO, gồm H2 và H3-H6. Dùng khi màn hình không muốn chấm heading. |
| `maxWordsOfTitle` | `17` | Số từ tối đa cho title SEO. Rule `TITLE_WORDS` dùng giá trị này để chấm title quá dài hay hợp lệ. |
| `maxWordsOfSapo` | `50` | Số từ tối đa cho description SEO. Rule `DESCRIPTION_WORDS` dùng giá trị này để chấm mô tả quá dài hay hợp lệ. |
| `ignoreTooLongSentences` | `false` | Bỏ qua rule tỷ lệ câu dài hơn 20 từ. |
| `ignoreExternalLinks` | `false` | Bỏ qua rule kiểm tra link ra ngoài. |
| `ignoreCheckmissingAlt` | `false` | Bỏ qua rule kiểm tra ảnh thiếu alt. |
| `ignorePercentTransitionWord` | `false` | Bỏ qua rule tỷ lệ câu có từ chuyển tiếp. |
| `ignoreKeywordInAlt` | `false` | Bỏ qua rule alt ảnh có chứa từ khóa chủ đạo hay không. |
| `ignorePercentKeywordCount` | `false` | Field giữ để tương thích luồng cũ. Hiện rule mật độ từ khóa nên bật/tắt bằng `criteria.CONTENT_PERCENT_KEYWORD`. |
| `ignoreHeadingH3ToH6` | `false` | Bỏ qua 2 rule liên quan đến heading H3-H6. |
| `usePercentKeywordCount` | `false` | Khi `true`, rule `CONTENT_KEYWORD` dùng mật độ từ khóa theo phần trăm thay vì số lần xuất hiện. |
...
| Tiêu chí | Default | Ý nghĩa |
| --- | --- | --- |
| `TITLE_KEYWORD` | `true` | Kiểm tra title SEO có chứa từ khóa chủ đạo hay không. |
| `TITLE_WORDS` | `true` | Kiểm tra số từ của title SEO có nằm trong ngưỡng hợp lệ không. Ngưỡng tối đa lấy từ `settings.maxWordsOfTitle`. |
| `TITLE_CHARACTERS` | `false` | Kiểm tra số ký tự của title SEO có nằm trong khoảng tối ưu không. |
| `DESCRIPTION_KEYWORD` | `true` | Kiểm tra description SEO có chứa từ khóa chủ đạo hay không. |
| `DESCRIPTION_WORDS` | `true` | Kiểm tra số từ của description SEO có nằm trong ngưỡng hợp lệ không. Ngưỡng tối đa lấy từ `settings.maxWordsOfSapo`. |
| `DESCRIPTION_CHARACTERS` | `false` | Kiểm tra số ký tự của description SEO có nằm trong khoảng tối ưu không. |
| `HEADING_H2_EXISTS` | `true` | Kiểm tra bài viết có ít nhất một thẻ H2 hay không. |
| `HEADING_H2_KEYWORD` | `true` | Kiểm tra từ khóa chủ đạo có xuất hiện trong H2 hay không. |
| `HEADING_H3_H6_EXISTS` | `false` | Kiểm tra bài viết có thẻ H3 đến H6 hay không. |
| `HEADING_H3_H6_KEYWORD` | `false` | Kiểm tra từ khóa chủ đạo có xuất hiện trong H3 đến H6 hay không. |
| `PRIMARY_KEYWORD_EXISTS` | `false` | Kiểm tra người dùng đã nhập từ khóa chủ đạo hay chưa. |
| `CONTENT_FIRST_PARAGRAPH_KEYWORD` | `false` | Kiểm tra đoạn đầu tiên của bài viết có chứa từ khóa chủ đạo hay không. |
| `CONTENT_KEYWORD` | `true` | Kiểm tra từ khóa chủ đạo có xuất hiện trong nội dung hay không. |
| `CONTENT_PERCENT_KEYWORD` | `false` | Kiểm tra mật độ từ khóa có nằm trong khoảng 1% đến 5% hay không. Mức 3% là tối ưu nhất. |
| `CONTENT_WORD_COUNT` | `true` | Kiểm tra nội dung có đạt tối thiểu 300 từ hay không. |
| `CONTENT_PARAGRAPH_LENGTH` | `true` | Kiểm tra bài viết có đoạn văn nào dài quá 300 từ hay không. |
| `CONTENT_LONG_SENTENCES` | `false` | Kiểm tra tỷ lệ câu dài hơn 20 từ có vượt 25% hay không. |
| `CONTENT_TRANSITION_WORD` | `false` | Kiểm tra tỷ lệ câu có từ chuyển tiếp có đạt 30% hay không. |
| `CONTENT_INTERNAL_LINK` | `false` | Kiểm tra bài viết có link nội bộ hay không. |
| `CONTENT_EXTERNAL_LINK` | `false` | Kiểm tra bài viết có link ra ngoài hay không. |
| `CONTENT_IMAGE_EXISTS` | `false` | Kiểm tra bài viết có hình ảnh hay không. |
| `CONTENT_IMAGE_ALT` | `false` | Kiểm tra tất cả hình ảnh có alt hay không. Nếu bật tính năng đồng bộ alt, cần thêm `source.syncCorrectImageAlt.set(html)`. |
| `CONTENT_KEYWORD_IN_ALT` | `false` | Kiểm tra alt ảnh có chứa từ khóa chủ đạo hay không. |
